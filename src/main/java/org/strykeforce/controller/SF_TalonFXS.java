...
public class SF_TalonFXS {
    ...
    public void runOpenLoop(double setpoint) {
        switch (openLoopUnits) {
            case Percent -> talonFXS.setControl(dutyCycleOut.withOutput(setpoint));
            case Voltage -> talonFXS.setControl(voltageOut.withOutput(setpoint));
            case Torque_Current -> talonFXS.setControl(torqueCurrentFOC.withOutput(setpoint));
        }
    }
    ...
    public void runDifferential(double target, double offset) {
        switch (differentialType) {
            case Follower -> {
                switch (followerType) {
                    case Standard -> talonFXS.setControl(
                            differentialFollower.withMasterID(leaderID).withOpposeMasterDirection(opposeMain));
                    case Strict -> talonFXS.setControl(differentialStrictFollower.withMasterID(leaderID));
                }
            }
            case Open_Loop -> {
                switch (openLoopUnits) {
    ...
    public void setupFollower(int leaderID) {
        ...
        switch (followerType) {
            case Standard -> talonFXS.setControl(follower.withMasterID(leaderID));
            case Strict -> talonFXS.setControl(strictFollower.withMasterID(leaderID));
        }
    }
    ...
    public void setupDifferentialFollower(int leaderID) {
        this.leaderID = leaderID;
        switch (followerType) {
            case Standard -> talonFXS.setControl(differentialFollower.withMasterID(leaderID));
            case Strict -> talonFXS.setControl(differentialStrictFollower.withMasterID(leaderID));
        }
    }
