...
class SSD1306 {
  ...
  val width =
    when (rotation) {
      Rotation.DEG_90,
      Rotation.DEG_270 -> DISPLAY_HEIGHT
      Rotation.DEG_0,
      Rotation.DEG_180 -> DISPLAY_WIDTH
    }

  val height =
    when (rotation) {
      Rotation.DEG_90,
      Rotation.DEG_270 -> DISPLAY_WIDTH
      Rotation.DEG_0,
      Rotation.DEG_180 -> DISPLAY_HEIGHT
    }
  ...
  fun clear() {
    synchronized(this) { Arrays.fill(writeBuffer, 0x00.toByte()) }
  }

  fun update() {
    synchronized(this) { System.arraycopy(writeBuffer, 0, readBuffer, 0, writeBuffer.size) }
    val setupCommands =
