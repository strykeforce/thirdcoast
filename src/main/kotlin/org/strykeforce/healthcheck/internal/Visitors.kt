...
class DumpVisitor : HealthCheckVisitor {
  ...
  override fun visit(healthCheck: TalonHealthCheckCase) {
    ...
    healthCheck.data.forEach {
      buffer.appendLine(
        "                    talon ${it.id} avg. voltage        = ${it.averageVoltage.format()} volts"
      )
      buffer.appendLine(
        "                    talon ${it.id} avg. speed          = ${it.averageSpeed.format()} ticks/100ms"
      )
      buffer.appendLine(
        "                    talon ${it.id} avg. supply current = ${it.averageSupplyCurrent.format()} amps"
      )
      buffer.appendLine(
        "                    talon ${it.id} avg. stator current = ${it.averageStatorCurrent.format()} amps"
      )
    }
  }
