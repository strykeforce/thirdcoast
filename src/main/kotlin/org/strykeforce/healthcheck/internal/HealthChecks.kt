...
abstract class HealthCheckCollection(
...
class RobotIOHealthCheck(name: String, healthChecks: List<IOHealthCheck>) :
  HealthCheckCollection(name, healthChecks) {
  override fun accept(visitor: HealthCheckVisitor) {
    visitor.visit(this)
  }
...
class IOHealthCheck(name: String, healthChecks: List<HealthCheck>) :
  HealthCheckCollection(name, healthChecks) {
  override var isFinished = false
...
class TalonFollowerHealthCheck(talon: BaseTalon, val leaderId: Int) :
  TalonHealthCheck(talon, listOf())

class P6TalonFollowerHealthCheck(talonFX: TalonFX, val leaderId: Int) :
  P6TalonHealthCheck(talonFX, listOf())

class FXSTalonFollowerHealthCheck(talonFXS: TalonFXS, val leaderId: Int) :
  FXSTalonHealthCheck(talonFXS, listOf())

class LifecycleHealthCheck(private val subsystem: Subsystem, private val method: Method) :
  HealthCheck {
...
abstract class P6TalonHealthCheckCase(
  ...
  val data: MutableList<P6TalonHealthCheckData> =
    mutableListOf(P6TalonHealthCheckData(case, talonFx))
...
abstract class FXSTalonHealthCheckCase(
  ...
  val data: MutableList<FXSTalonHealthCheckData> =
    mutableListOf(FXSTalonHealthCheckData(case, talonFxs))
