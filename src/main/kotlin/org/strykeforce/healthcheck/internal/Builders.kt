...
private val kBeforeHealthCheckAnnotationClass =
  org.strykeforce.healthcheck.BeforeHealthCheck::class.java
private val kAfterHealthCheckAnnotationClass =
  org.strykeforce.healthcheck.AfterHealthCheck::class.java

/**
 * Get fields in subsystem that are annotated with `@HealthCheck`. The fields are sorted in order of
 * the `@HealthCheck` annotation's `order` parameter.
 */
private fun Subsystem.healthCheckAnnotatedFields() =
  this.javaClass.declaredFields
    .filter { it.isAnnotationPresent(kHealthCheckAnnotationClass) }
...
/**
 * Get methods in subsystem that are annotated with `@BeforeHealthCheck`. The methods are sorted in
 * order of the method's name.
 */
private fun Subsystem.beforeHealthCheckAnnotatedMethods() =
  this.javaClass.declaredMethods
    .filter { it.isAnnotationPresent(kBeforeHealthCheckAnnotationClass) }
...
/**
 * Get methods in subsystem that are annotated with `@AfterHealthCheck`. The methods are sorted in
 * order of the method's name.
 */
private fun Subsystem.afterHealthCheckAnnotatedMethods() =
  this.javaClass.declaredMethods
    .filter { it.isAnnotationPresent(kAfterHealthCheckAnnotationClass) }
...
class SubsystemHealthCheckBuilder(private val subsystem: Subsystem) {
  ...
  fun build(): SubsystemHealthCheck {
    ...
    // create LifecycleHealthChecks from annotated methods and prepend them to health checks for
    // this subsystem
    healthChecks.addAll(
      beforeHealthCheckMethods.map { LifecycleHealthCheckBuilder(subsystem, it).build() }
    )
    ...
    //        healthChecks.addAll(healthCheckFields.map { TalonHealthCheckBuilder(subsystem,
    // it).build() })
    ...
    for (fhc in followerHealthChecks) {
      val lhc =
        healthChecks
          .mapNotNull { it as? TalonHealthCheck }
          .find { it.talon.deviceID == fhc.leaderId }
      if (lhc == null) {
        logger.error {
          "$subsystem: no leader (id = ${fhc.leaderId}) found for follower (id = ${fhc.talon.deviceID})"
        }
        continue
      }
    ...
    // create LifecycleHealthChecks from annotated methods and append them to health checks for this
    // subsystem
    healthChecks.addAll(
      afterHealthCheckMethods.map { LifecycleHealthCheckBuilder(subsystem, it).build() }
    )
...
class IOHealthCheckBuilder(private val io: Checkable) {
  fun build(): IOHealthCheck {
    ...
    // this io layer
    healthChecks.addAll(
      beforeHealthCHeckMethods.map { LifecycleHealthCheckIOBuilder(io, it).build() }
    )
    ...
    for (fhc in followerHealthChecks) {
      ...
      if (lhc == null) {
        logger.error {
          "$io: no leader(id = ${fhc.leaderId}) found for follower (id = ${fhc.talon.deviceID})"
        }
        continue
      }
    ...
    for (fhc in p6FollowerHealthChecks) {
      val lhc =
        healthChecks
          .mapNotNull { it as? P6TalonHealthCheck }
          .find { it.talonFx.deviceID == fhc.leaderId }
      if (lhc == null) {
        logger.error {
          "$io: no leader (id = ${fhc.leaderId}) found for follower (id = ${fhc.talonFx.deviceID})"
        }
        continue
      }
      // leader is timed or position check, add follower to each child case
      lhc.healthChecks
        .map { it as P6TalonHealthCheckCase }
        .forEach { it.addFollowerTalon(fhc.talonFx) }
    }
    ...
    for (fhc in fxsFollowerHealthChecks) {
      val lhc =
        healthChecks
          .mapNotNull { it as? FXSTalonHealthCheck }
          .find { it.talonFxs.deviceID == fhc.leaderId }
      if (lhc == null) {
        logger.error {
          "$io: no leader (id = ${fhc.leaderId}) found for follower (id = ${fhc.talonFxs.deviceID})"
        }
        continue
      }
      // leader is timed or position check, add follower to each child case
      lhc.healthChecks
        .map { it as FXSTalonHealthCheckCase }
        .forEach { it.addFollowerTalon(fhc.talonFxs) }
    }
    ...
    // create lifecycleHealthCheck from annotated methods and append them to health checks for this
    // io layer
    healthChecks.addAll(
      afterHealthCheckMethods.map { LifecycleHealthCheckIOBuilder(io, it).build() }
    )
...
class LifecycleHealthCheckIOBuilder(private val io: Checkable, private val method: Method) {
  fun build(): HealthCheck {
    ...
    if (!method.trySetAccessible()) {
      val msg =
        "$ioName.${method.name}: inaccessible, crashing to prevent health check running without setup"
      logger.error { msg }
...
class SwerveDriveHealthCheckBuilder(private val subsystem: Subsystem, private val field: Field) {
  ...
  private val swerveDrive =
    field.get(subsystem) as? SwerveDrive
  ...
  fun build(): List<HealthCheck> {
    ...
    swerveDrive.swerveModules.forEach {
      val module =
        it as? V6TalonSwerveModule
          ?: throw IllegalArgumentException("$subsystem: $it is not TalonSwerveModule")
    ...
    requireNotNull(azimuthLeader) {
      "$subsystem: swerve azimuth talon with id $AZIMUTH_LEADER_ID not found"
    }
    requireNotNull(driveLeader) {
      "$subsystem: swerve drive talon with id $DRIVE_LEADER_ID not found"
    }

    azimuthFollowers.forEach { it.follow(azimuthLeader) }
    driveFollowers.forEach {
      it.setControl(Follower(driveLeaderId ?: DRIVE_LEADER_ID, MotorAlignmentValue.Aligned))
    }
...
class SwerveDriveHealthCheckIOBuilder(private val io: Checkable, private val field: Field) {
  private val swerveDrive =
    field.get(io) as? SwerveDrive
      ?: throw IllegalArgumentException("$io: field '${field.name}' is not a SwerveDrive")
  ...
  fun build(): List<HealthCheck> {
    ...
    swerveDrive.swerveModules.forEach {
      val module =
        it as? V6TalonSwerveModule
          ?: throw IllegalArgumentException("$io: $it is not TalonSwerveModule")
      val azimuth = module.azimuthTalon
      val drive = module.driveTalon
    ...
    requireNotNull(azimuthLeader) {
      "$io: swerve azimuth talon with id $AZIMUTH_LEADER_ID not found"
    }
...
class FXSwerveDriveHealthCheckIOBuilder(private val io: Checkable, private val field: Field) {
  ...
  fun build(): List<HealthCheck> {
    ...
    azimuthFollowers.forEach {
      it.setControl(Follower(azimuthLeaderId ?: AZIMUTH_LEADER_ID, MotorAlignmentValue.Aligned))
    }
    driveFollowers.forEach {
      it.setControl(Follower(driveLeaderId ?: DRIVE_LEADER_ID, MotorAlignmentValue.Aligned))
    }
...
class TalonHealthCheckBuilder(private val subsystem: Subsystem, private val field: Field) {
  ...
  val talon =
    field.get(subsystem) as? BaseTalon
      ?: throw IllegalArgumentException(
        "$subsystem: field '${field.name}' is not a subclass of BaseTalon"
      )
  ...
  fun build(): TalonHealthCheck {
    ...
    if (
      ...
      throw IllegalArgumentException(
        "$subsystem: only one of @Timed, @Position, or @Follow may be specified."
      )
...
class TalonHealthCheckIOBuilder(private val io: Checkable, private val field: Field) {
  val talon =
    field.get(io) as? BaseTalon
      ?: throw IllegalArgumentException("$io: field '${field.name}' is not a subclass of BaseTalon")
...
class P6TalonHealthCheckBuilder(private val subsystem: Subsystem, private val field: Field) {
  val talonFx =
    field.get(subsystem) as? TalonFX
      ?: throw java.lang.IllegalArgumentException(
        "$subsystem: field '${field.name}' is not subclass of TalonFX"
      )
...
class P6TalonHealthCheckIOBuilder(private val io: Checkable, private val field: Field) {
  val talonFx =
    field.get(io) as? TalonFX
      ?: throw java.lang.IllegalArgumentException(
        "$io: field '${field.name}' is not subclass of TalonFX"
      )
  ...
  fun build(): TalonHealthCheck {
    val cases =
      percentOutput.let {
        ...
        it.map { pctOut ->
          val case =
            TalonTimedHealthCheckCase(previousCase, talon, pctOut, (duration * 1e6).toLong())
          previousCase = case
          case
        }
      }
  ...
  fun build(): P6TalonHealthCheck {
    val cases =
      percentOutput.let {
        ...
        it.map { pctOut ->
          val case =
            P6TalonTimedHealthCheckCase(previousCase, talonFx, pctOut, (duration * 1e6).toLong())
          previousCase = case
          case
        }
      }
  ...
  fun build(): FXSTalonHealthCheck {
    val cases =
      percentOutput.let {
        ...
        it.map { pctOut ->
          val case =
            FXSTalonTimedHealthCheckCase(previousCase, talonFxs, pctOut, (duration * 1e6).toLong())
          previousCase = case
          case
        }
      }
...
class TalonPositionHealthCheckBuilder(val talon: BaseTalon) {
  ...
  fun build(): TalonHealthCheck {
    if (encoderChange == 0)
      logger.warn { "Talon ${talon.deviceID}: position health check encoderChange is zero" }
...
class P6TalonPositionHealthCheckBuilder(var talonFx: TalonFX) {
  ...
  fun build(): P6TalonHealthCheck {
    if (encoderChange == 0.0)
      logger.warn { "Talon ${talonFx.deviceID}: position health check encoderChange is zero" }
...
class FXSTalonPositionHealthCheckBuilder(var talonFxs: TalonFXS) {
  ...
  fun build(): FXSTalonHealthCheck {
    if (encoderChange == 0.0)
      logger.warn { "Talon ${talonFxs.deviceID}: position health check encoderChange is zero" }
