...
internal constructor(
  ...
  class Builder(private val type: SignalType) {
    ...
    fun build(): SignalGenerator =
      when (type) {
        SignalGenerator.SignalType.SINE ->
          SineSignalGenerator(frequency, phase, amplitude, offset, invert)
        SignalGenerator.SignalType.SQUARE ->
          SquareSignalGenerator(frequency, phase, amplitude, offset, invert)
        SignalGenerator.SignalType.TRIANGLE ->
          TriangleSignalGenerator(frequency, phase, amplitude, offset, invert)
        SignalGenerator.SignalType.SAWTOOTH ->
          SawtoothSignalGenerator(frequency, phase, amplitude, offset, invert)
      }
  }
}
